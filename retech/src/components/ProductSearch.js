...
const ProductSearch = () => {
    ...
    const handleNameSearch = async () => {
        try {
            const response = await fetch('http://localhost:4002/b2/products/search', {
                method: 'POST',
                headers: {
            ...
            // Filter out inactive product from search results
            ...
            console.error('Error searching for active products by name:', error);
        }
    };

    const handlePriceSearch = async () => {
        try {
            const response = await fetch('http://localhost:4002/b2/products/searchByPrice', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            if (activeProducts.length === 0) {
                Swal.fire({
                    title: 'Product Not Available',
                    icon: 'error',
                    text: 'No products available in the specified price range.',
                });
            }
